...
- Client 手牌整理已支持排序、手动移动、长按多选、滑动连选和大手牌密度处理，后续仍需继续打磨极端手牌数量下的触控手感。
- Host 桌面动作需要更多防误触和可解释反馈。
- PWA 安装基础和版本更新提示已有第一版，刷新恢复和更完整的局设置还需要补齐。
...
- Client 摸牌区支持选择摸 1/2/3/5 张，Host 仍按权威牌堆实际剩余数裁剪。
- Host 和手机 Host 都可以移除离线玩家，并将其手牌回收到牌堆。
- Host 面板展示最近 move ledger 时间线，用于解释牌流和调试容器转移。
- move ledger 条目带有递增序号、card id 列表和 replayable 标记，便于后续回放与审计。
- Client 撤销按钮改为由 Host move ledger 推导，未批准的 pending 请求不会提前计入可撤销次数。
- PWA service worker 有新版可用时会显示更新提示，避免静默缓存影响联机。
- Game Pack 配置已有第一版 typed schema，标准沙盒包可选择 52/54 张标准扑克预设。
- Game Pack schema 已预留非标准卡牌字段：牌名、类别、标签、face id，并有一个不含真实 IP 素材的“类三国杀”占位预设。
- Host 与手机 Host 已提供局设置入口：牌组预设、起始发牌数、是否允许抽其他玩家暗牌、是否允许玩家清桌、是否允许玩家撤销。
- 重置整局会按当前牌组预设重新生成权威牌堆，起始发牌可由 Host 按设置一键执行。
...
- 牌背资源。
- 牌库列表和 card id 映射。
- 卡牌类别，例如普通扑克、基础牌、锦囊/策略牌、装备牌、身份牌、角色牌、自定义牌。
- 卡牌展示元数据，例如标题、标签、face id 和资源引用。
- 桌面容器定义。
- 默认玩家人数。
- 默认座位布局。
- 默认发牌数。
...
三国杀类游戏应优先通过 Game Pack 表达身份、角色、装备、判定、血量等公开信息，不应把完整规则硬编码进核心牌流。
...
- `[done] Client: add draw N cards action`
- `[done] Client: add long press multi-select and clear selection`
- `[done] Host: confirm dangerous reset and clear actions`
- `[done] PWA: add update available prompt`
- `[done] Room: add host controls for offline players and reconnect messaging`
- `[done] Host: replace seat dropdown with tap-table seat assignment and release-seat controls`
- `[partial] MoveLedger: make ledger replayable beyond undo and expose a debug timeline`
- `[done] Client: improve pending action state and undo count accuracy for unapproved requests`

第三批：

- `[done] GamePack: define JSON schema`
- `[done] GamePack: support standard 52 and 54 card presets`
- `[done] GamePack: add hero-duel prototype for 三国杀-like card categories and zones`
- `[partial] GamePack: make table containers configurable`
- `[done] Room: add host game settings for deck preset, starting hand, clear, steal, and undo permissions`
